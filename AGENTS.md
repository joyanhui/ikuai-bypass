...
- **更新与安全策略**: 
  - **原地更新 (ID 稳定性)**: 采取“先查询、匹配则 Edit、不匹配则 Add”的策略。通过维护 `序号/名字 -> ID` 映射，确保爱快内部 ID 稳定，避免依赖 ID 的外部规则失效。
  - **ISP 分片迁移兼容性（仅自定义运营商）**: ISP的规则要求 分片后的规则名字 采取 IKB名字  不可以带序号，但是备注里面有序号，查询的时候也需要查看备注里面的序号判断 是否匹配 更新/edit 不匹配 add，冗余删除 的策略
  - **故障停机保护 (Safe-Before)**: 远程资源下载失败或 HTTP 状态码异常时，必须立即终止当前项更新，**严禁执行删除/清理操作**，确保旧规则持续有效。故障应隔离在当前模块，不影响其他配置项。
  - **冗余清理日志**: 删除多余分片时，必须清晰记录被删除规则的完整名字、序号及爱快内部 ID。
  - **配置校验**: 配置文件的覆写必须经过后缀名和软链接校验，严禁写入非 YAML 格式内容。
