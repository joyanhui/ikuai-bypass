# iKuai Bypass
这是一个专门为 爱快路由器开发的自动化分流规则同步工具。它通过模拟 Web 管理界面的行为，将远程订阅的 IP/域名列表同步到路由器的分流设置中。目前支持：
- 运营商分流/ip分流（支持ipv6）
- 域名分流
- 端口分流/下一条网关
- 计划任务模式
...
- v4.0.1 重构了项目结构，修复了端口分流只保留一条的bug[#96](https://github.com/joyanhui/ikuai-bypass/issues/96)；优化了IP和IPv6分组更新流程[#97](https://github.com/joyanhui/ikuai-bypass/pull/97)；增加了Docker支持。
### ip分组和端口分流方式 说明：
