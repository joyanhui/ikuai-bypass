...
- [重要] v4.4 以后不再支持爱快 v3.x 版本（v3.7 及以下请用 v4.2）
- [重要] 规则标识从备注改为名字前缀 IKB（v4 接口备注无返回）
- [重要] 配置项统一使用 tag，删除 name 等混乱字段，参考 config.yml 更新配置
...
- [重要] 清理模式必须显式配置 `-tag` 参数，不再默认 CleanAll，仅清理名字包含 IKB 开头的规则（当前统一备注为 `IkuaiBypass`，兼容旧版备注 `IKUAI_BYPASS` 或 `joyanhui/ikuai-bypass`）
- IP分组去掉注释功能，因为爱快v4的ip分组功能的注释实际上是行注释已经失去原作用
- 修复 v4.4.2-Pre 自定义运营商的 bug [#105](https://github.com/joyanhui/ikuai-bypass/issues/105)
- 增加配置可以控制单条分组/分流规则的最大记录数量，可以绕开爱快的限制，分别增加50%或者数倍。参考 [#105](https://github.com/joyanhui/ikuai-bypass/issues/105)，参考 `config.yml` 第67行
