...
impl Config {
    ...
    fn to_commented_yaml(&self) -> String {
        ...
        out.push('\n');
        out.push_str("  enable-update: ");
        out.push_str(if self.webui.enable_update {
            "true"
        } else {
            "false"
        });
...
pub fn webui_comments() -> BTreeMap<String, String> {
    BTreeMap::from([
        ...
        ("enable".to_string(), "是否启用 WebUI 服务".to_string()),
        (
            "enable-update".to_string(),
            "是否启用配置文件在线更新功能".to_string(),
        ),
        (
